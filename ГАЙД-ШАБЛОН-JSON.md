...
### 4.13. Безопасность сессий и API-токенов

Требования апробирования ИБ к управлению сессиями и токенами API.

#### Неактивность сессии (блокировка и повторная аутентификация)

Должна обеспечиваться настройка **длительности периода неактивности** для автоматической блокировки сессии; **восстановление сессии — только после повторной аутентификации**.

| Где настраивать | Что сделать |
|-----------------|-------------|
| Портал администратора | «Система» → «Системные настройки» → разделы безопасности/сессий (название зависит от сборки 7.0). Задайте таймаут неактивности веб-сессии. |
| Политики / группы | Политики безопасности для пользовательского портала и клиентов Connect. |
| termidesk.conf | При необходимости — параметры Django/сессий по документации вашей сборки (сверяйте с `termidesk-config`). |

**Проверка на стенде:** войти в портал → не выполнять действий дольше заданного таймаута → убедиться, что запросы отклоняются и требуется новый логин (не «тихое» продолжение без пароля).

#### Срок действия токена API

Должна обеспечиваться возможность **установки срока действия токена** при доступе через API (либо **перегенерация токена после повторной аутентификации**).

| Механизм | Параметр / место | Примечание |
|----------|------------------|------------|
| JWT Агрегатора | `AGGREGATOR_ACCESS_TOKEN_TTL_SECONDS` (по умолчанию 600 с) | На узлах с Агрегатором и связанных диспетчерах |
| REST API портала | `POST /api/auth/v7.0/login` → поле `token` | Срок жизни задаётся политикой сервера |
| OpenBao | `SECRETS_OPENBAO_WRAP_TTL`, watchdog в termidesk-config | Токены доступа к секретам |

> **Требует уточнения на стенде:** сейчас типичное поведение — **сессия в веб-интерфейсе разрывается по таймауту неактивности, но выданный API-токен (`Bearer`) может оставаться действительным до истечения собственного TTL**. Для соответствия ИБ необходимо:
> 1. Зафиксировать фактическое время жизни токена после `login`.
> 2. Согласовать с эксплуатацией: уменьшить TTL, включить принудительный logout API, либо документировать компенсирующие меры (ротация, WAF, отзыв через смену пароля).
> 3. Повторный `login` после истечения веб-сессии должен выдавать **новый** токен; старый — не приниматься.

Скрипт проверки health/metrics API: `output/19-ib-compliance/check-health-api.ps1` (пункт меню **[19]**).
